                       ...
                       CFTypeRef attrValue) // IN: The data to convert
{
    ...
    if (AXUIElementGetTypeID() == CFGetTypeID(attrValue)) {
        CFRetain(attrValue);
        return _newAXUIElementWithRef(attrValue, Py_TYPE(self));
    }
        ...
        for (i=0; i<count; i++) {
            ...
            if (NULL != element) {
                if (AXUIElementGetTypeID() == CFGetTypeID(element)) {
                    CFRetain(element);
                    temp = _newAXUIElementWithRef(element, Py_TYPE(self));
                    if (NULL == temp) {
...
// tuple
{
    ...
    return _newAXUIElementWithRef(res, Py_TYPE(self));
                 ...
                 void *contextData)          // IN: context data
{
    ...
    if (NULL != axObj->callbackFn) {
        ...
        if (0 != PyTuple_Size(callbackArgs)) {
            ...
            CFRetain(element);
            retElem = _newAXUIElementWithRef(element, Py_TYPE(axObj));
            if (NULL == retElem) {
...
static struct PyModuleDef moduledef = {
    ...
    "atomac._a11y",     /* m_name */
    "Library of Apple A11y functions",  /* m_doc */
    -1,                  /* m_size */
    a11ylibMethods,    /* m_methods */
    NULL,                /* m_reload */
    NULL,                /* m_traverse */
    NULL,                /* m_clear */
    NULL,                /* m_free */
...
#endif
{
    ...
    #else
        m = Py_InitModule3("atomac._a11y", &moduledef,
    ...
    atomacError = PyErr_NewException("atomac._a11y.Error", PyExc_Exception,
                                     NULL);
